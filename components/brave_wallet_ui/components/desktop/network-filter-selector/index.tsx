...
export const NetworkFilterSelector = ({
  ...
  const reduxNetworkList = getEntitiesListFromEntityState(
    networksRegistry,
    networksRegistry.visibleIds
  )
  const selectedNetworkFilterId = networkEntityAdapter.selectId(selectedNetworkFilter)
  ...
  const sortedNetworks = React.useMemo(() => {
    const onlyMainnets = filteredNetworks.filter((network) => SupportedTopLevelChainIds.includes(network.chainId))
    const removedMainnets = filteredNetworks.filter((network) => !SupportedTopLevelChainIds.includes(network.chainId))
  ...
  const testNetworks = React.useMemo(() => {
    return filteredNetworks.filter((network) => SupportedTestNetworks.includes(network.chainId))
  }, [filteredNetworks])
