...
const uint8_t kV0MessagePrefix = 0x80;
const uint8_t kVersionPrefixMask = 0x7f;
...
base::Value::Dict SolanaMessage::ToValue() const {
  ...
  dict.Set("version", static_cast<int32_t>(version_));
  ...
  dict.Set("message_header", message_header_.ToValue());

  base::Value::List static_account_key_list;
  for (const auto& static_account_key : static_account_keys_) {
    static_account_key_list.Append(static_account_key.ToBase58());
  }
  dict.Set("static_account_keys", std::move(static_account_key_list));
  ...
  dict.Set("address_table_lookups", std::move(address_table_lookup_list));
...
absl::optional<SolanaMessage> SolanaMessage::FromValue(
  ...
  auto version = value.FindInt("version");
  ...
  const base::Value::Dict* message_header_dict =
      value.FindDict("message_header");
  ...
  const base::Value::List* static_account_key_list =
      value.FindList("static_account_keys");
  ...
  const base::Value::List* address_table_lookup_list =
      value.FindList("address_table_lookups");
