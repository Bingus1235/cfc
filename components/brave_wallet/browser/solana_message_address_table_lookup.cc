...
absl::optional<std::vector<uint8_t>> GetIndexesFromBase64EncodedPrefString(
    const base::Value::Dict& value,
    const std::string& pref_key) {
  const std::string* base64_encoded_indexes = value.FindString(pref_key);
...
SolanaMessageAddressTableLookup::FromValue(const base::Value::Dict& value) {
  ...
  auto write_indexes =
      GetIndexesFromBase64EncodedPrefString(value, kBase64EncodedWriteIndexes);
  if (!write_indexes) {
    return absl::nullopt;
  }

  auto read_indexes =
      GetIndexesFromBase64EncodedPrefString(value, kBase64EncodedReadIndexes);
  if (!read_indexes) {
    return absl::nullopt;
  }
