...
TEST(SolanaMessageHeaderUnitTest, FromToValue) {
  ...
  EXPECT_EQ(header, SolanaMessageHeader::FromValue(value.GetDict()));
  EXPECT_EQ(value.GetDict(), header.ToValue());
  ...
  for (const auto& invalid_value_string : invalid_value_strings) {
    ...
    EXPECT_FALSE(SolanaMessageHeader::FromValue(invalid_value.GetDict()))
