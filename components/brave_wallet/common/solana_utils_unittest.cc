...
TEST(SolanaUtilsUnitTest, GetUint8FromStringPref) {
  ...
  EXPECT_EQ(GetUint8FromStringPref(dict, "valid").value(), UINT8_MAX);
  EXPECT_FALSE(GetUint8FromStringPref(dict, "too_big"));
  EXPECT_FALSE(GetUint8FromStringPref(dict, "not_a_number"));
}
